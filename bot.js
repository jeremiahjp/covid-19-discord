...
function sendFoundCovidResponse(msg, covid_data, service_type) {
 ...
 --------------|----------------- 
  ...
  const embed = new Discord.RichEmbed()
    .setTitle(
      `Coronavirus COVID-19 Cases by the Center for Systems Science and Engineering (CSSE) at Johns Hopkins University (JHU)`
    )
    .setColor(Math.floor(Math.random() * 16777214 - 0 + 1) + 0)
    .setAuthor("iSuck Bot")
...
client.on("message", async (msg) => {
  ...
  if (msg.content.substring(0, 8) === "!covid19") {
    ...
    const HOPKINS_STATE_PROVINCE_FILENAME =
      "hopkins-latest-state-province.json";

    if (
      args.length <= 1 ||
      (!STATE_PROVINCE_COMMAND.includes(command_type.toLowerCase()) &&
        command_type.toLowerCase() !== COUNTRY_COMMAND)
    ) {
